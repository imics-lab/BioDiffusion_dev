...
def train(args):
    ...
    if args.all_class:
        simulated = datasets.simulated_data(file_folder = args.data_path, all_class=True)
    else:
        simulated = datasets.simulated_data(file_folder = args.data_path, class_id = args.class_id)
    dataloader = data.DataLoader(simulated, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run-name', type=str, default='DDPM1D_Uncondtional_SIMULATED', help='Description of the run name')
