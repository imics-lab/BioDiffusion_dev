...
class unimib_load_dataset(Dataset):
    def __init__(self, 
        ...
        incl_val_group = False, #True => returns x/y_test, x/y_validation, x/y_train
                               #False => combine train & validation groups
        is_normalize = True,
        split_subj = dict
                    (train_subj = [4,5,6,7,8,10,11,12,14,15,19,20,21,22,24,26,27,29],
                    validation_subj = [1,9,16,23,25,28],
                    test_subj = [2,3,13,17,18,30]),
        one_hot_encode = False, data_mode = 'Train', single_class = False, class_name= 'Walking', augment_times = None):
...
class syn_unimib(Dataset):
    def __init__(self, file_folder = './synthetic/synthetic_data/', n_samples = 1000):
        self.syn_cl0s = np.load(os.path.join(file_folder, 'syn_StandingUpFS.npy'))
        self.syn_cl0s = resample(self.syn_cl0s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl1s = np.load(os.path.join(file_folder, 'syn_StandingUpFL.npy'))
        self.syn_cl1s = resample(self.syn_cl1s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl2s = np.load(os.path.join(file_folder, 'syn_Walking.npy'))
        self.syn_cl2s = resample(self.syn_cl2s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl3s = np.load(os.path.join(file_folder, 'syn_Running.npy'))
        self.syn_cl3s = resample(self.syn_cl3s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl4s = np.load(os.path.join(file_folder, 'syn_GoingUpS.npy'))
        self.syn_cl4s = resample(self.syn_cl4s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl5s = np.load(os.path.join(file_folder, 'syn_Jumping.npy'))
        self.syn_cl5s = resample(self.syn_cl5s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl6s = np.load(os.path.join(file_folder, 'syn_GoingDownS.npy'))
        self.syn_cl6s = resample(self.syn_cl6s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl7s = np.load(os.path.join(file_folder, 'syn_LyingDownFS.npy'))
        self.syn_cl7s = resample(self.syn_cl7s, n_samples=n_samples, random_state=123, replace=True)
        
        self.syn_cl8s = np.load(os.path.join(file_folder, 'syn_SittingDown.npy'))
        self.syn_cl8s = resample(self.syn_cl8s, n_samples=n_samples, random_state=123, replace=True)
        
        
        self.data = np.concatenate((self.syn_cl0s, self.syn_cl1s, self.syn_cl2s, self.syn_cl3s, self.syn_cl4s, self.syn_cl5s, self.syn_cl6s, self.syn_cl7s, self.syn_cl8s), axis = 0)
        self.labels = np.concatenate(([0]*n_samples, [1]*n_samples, [2]*n_samples, [3]*n_samples, [4]*n_samples, [5]*n_samples,[6]*n_samples,[7]*n_samples,[8]*n_samples), axis = 0)
        
        print(f'Data shape is {self.data.shape}')
        print(f'Label shape is {self.labels.shape}')
...
class mixed_unimib(Dataset):
    def __init__(self, n_samples = 1000, file_folder = './synthetic/synthetic_data/'):
        real = unimib_load_dataset()
        real_data = real.x_train
        real_labels = real.y_train
        
        syn = syn_unimib(file_folder = file_folder, n_samples = n_samples)
        syn_data = syn.data
        syn_labels = syn.labels
        
        self.data = np.concatenate((real_data, syn_data), axis = 0)
        self.labels = np.concatenate((real_labels, syn_labels), axis = 0)
        
        print(f'Mixed Data shape is {self.data.shape}')
        print(f'Mixed Label shape is {self.labels.shape}')
