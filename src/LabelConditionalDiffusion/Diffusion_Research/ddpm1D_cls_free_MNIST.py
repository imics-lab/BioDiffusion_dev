...
def train(args):
    ...
    device = args.device
    mnist = MNIST_unconditional(oneD = True)
    ...
    for epoch in range(args.epochs):
        ...
        save_images_1D_to_2D_cls_free(sampled_signals, labels, os.path.join("results", args.run_name, f"{epoch}.jpg"))
        save_checkpoint({
            'epoch': epoch + 1,
            'model': model,
...
def launch():
    ...
    args.run_name = "DDPM1D_cls_free_MNIST"
    args.epochs = 300
    args.batch_size = 64
    args.seq_length = 128
    args.num_classes = 10
